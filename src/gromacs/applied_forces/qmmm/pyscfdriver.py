...
QM_E_BASIS = 'def2-svpd'
DFT_E_XC = 'B3LYP'
QM_NUC_BASIS = 'pb4d'
QM_NUC_SELECT = 'all' # select from {'all', 'custom'}
QM_NUC_INDEX = [66]
QM_E_BASIS_AUX = 'def2-svp-jkfit'
MM_CHARGE_MODEL = 'gaussian' # select from {'point', 'gaussian'}
LATTICE = 100 * 10
RCUT_EWALD = None # if not set, rcut_ewald will be half the box size
RCUT_HCORE = 20
...
def qmmmCalc(
    ...
    if QM_METHOD.upper() == 'CNEO':
        [energy, qmforces, mmforces] = qmmmCalc_cneo(
            qmatoms, mmcoords, mmcharges, mmradii, qmnucindex=qmnucindex
        )
        ...
        [energy, qmforces, mmforces] = qmmmCalc_mulliken(
            qmatoms, mmcoords, mmcharges, mmradii
        )
        ...
        [energy, qmforces, mmforces] = qmmmCalc_dftb(
            qmcoords_link, mmcoords, mmcharges)
...
def qmCalc_cneo(qmatoms, qmnucindex):
    ...
    if DF_EE:
        mf = mf.density_fit(auxbasis=QM_E_BASIS_AUX, df_ne=DF_NE)
...
def qmmmCalc_cneo(qmatoms, mmcoords, mmcharges, mmradii, qmnucindex):
    ...
    mmmol = mm_mole.create_mm_mol(mmcoords, numpy.eye(3)*LATTICE, mmcharges,
                                  mmradii, rcut_ewald=RCUT_EWALD, rcut_hcore=RCUT_HCORE)
    mol_neo = neo.M(
        atom=qmatoms,
        basis=QM_E_BASIS,
        nuc_basis=QM_NUC_BASIS,
...
def qmmmCalc_dft(qmatoms, mmcoords, mmcharges, mmradii):
    ...
    mol = gto.M(atom=qmatoms,
                ...
                charge=QM_CHARGE,
                spin=QM_MULT-1)
    mf_dft = dft.KS(mol, xc=DFT_E_XC)

    if DF_EE:
        mf_dft = mf_dft.density_fit(auxbasis=QM_E_BASIS_AUX)

    mf = itrf.add_mm_charges(mf_dft, mmcoords, numpy.eye(3)*LATTICE, mmcharges,
                             mmradii, rcut_ewald=RCUT_EWALD, rcut_hcore=RCUT_HCORE)
    ...
    mmforces = -(g.grad_nuc_mm() + g.grad_hcore_mm(mf.make_rdm1()) + g.de_ewald_mm)
    tmf = time.time()
    print(f"time for mm force = {tmf - tqf} seconds")
    print(f"time for this step = {time.time() - t0} seconds")
