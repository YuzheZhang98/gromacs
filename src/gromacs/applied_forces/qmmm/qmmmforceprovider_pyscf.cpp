 ...
 void QMMMForceProvider::calculateForces(const ForceProviderInput& fInput, ForceProviderOutput* fOutput)
 {
     ...
     if (numAtomsMM > 0)
     {
         ...
         for (size_t i = 0; i < numLinks; i++)
         {
             ...
             {
                 if (qmAtoms_.globalIndex()[j] == parameters_.link_[i].getEmbeddedIndex())
                 {
             ...
             {
                 if (mmAtoms_.globalIndex()[j] == parameters_.link_[i].getMMIndex())
                 {
    ...
    fInput.mpiComm_.sumReduce(3 * numAtoms, x.data()->as_vec());
     ...
     if (fInput.mpiComm_.isMainRank())
     {
         ...
         fOutput->enerd_.term[InteractionFunction::QuantumMechanicalRegionEnergy] += qmEner * c_hartree2Kj * c_avogadro;
     }
